    ...
    try {
      ...
      if (error) {
        if (error instanceof multer.MulterError && error.code === 'LIMIT_FILE_SIZE') {
          return res.status(400).json({
            ...
            code: error.code,
            detail: 'Multer rechazó el archivo por tamaño máximo absoluto.'
          });
        }
        ...
        if (error instanceof multer.MulterError && error.code === 'LIMIT_UNEXPECTED_FILE') {
          return res.status(400).json({
            ...
            code: error.code,
            detail: 'El backend espera el archivo en el campo "file".'
          });
        }
        ...
        return res.status(400).json({
          ...
          detail: error.message || 'No se pudo procesar el archivo.'
        });
      }
      ...
      if (!req.file) {
        return res.status(400).json({
          ...
          code: 'NO_FILE',
          detail: 'Envía FormData con el campo "file".'
        });
      }
      ...
      if (req.file.size > maxSize) {
        ...
        return res.status(400).json({
          ...
          code: isVideo ? 'VIDEO_TOO_LARGE' : 'FILE_TOO_LARGE',
          detail: `Tamaño recibido: ${req.file.size} bytes`
        });
      }
      ...
      return res.status(500).json({
        ...
        detail: 'Error interno al guardar o validar el archivo.'
      });
    }
  });
});
